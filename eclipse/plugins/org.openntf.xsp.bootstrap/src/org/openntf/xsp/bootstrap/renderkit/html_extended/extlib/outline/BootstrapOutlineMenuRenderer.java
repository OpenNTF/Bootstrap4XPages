...
import org.openntf.xsp.bootstrap.renderkit.html_extended.extlib.outline.tree.BootstrapMenuRenderer;
...
public class BootstrapOutlineMenuRenderer extends AbstractOutlineRenderer {

	@Override
	protected ITreeRenderer findTreeRenderer(FacesContext context, AbstractOutline outline) {
		BootstrapMenuRenderer r = createMenuRenderer(context, outline);
	...
	protected BootstrapMenuRenderer createMenuRenderer(FacesContext context, AbstractOutline outline) {
		int type = BootstrapMenuRenderer.TYPE_PILL;
		if(outline!=null) {
			String styleClass = outline.getStyleClass();
			if(StringUtil.isNotEmpty(styleClass) && styleClass.contains("nav-list")) {
				type = BootstrapMenuRenderer.TYPE_LIST;
			}
		}
		return new BootstrapMenuRenderer(outline,type);
	}
}